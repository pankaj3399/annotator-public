      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 max-w-lg w-full text-center border border-white/20">
        ...
        <div className="mb-8 relative">
          ...
          <div className="absolute inset-2 w-20 h-20 mx-auto">
            <div className="w-full h-full bg-red-300 rounded-full animate-ping opacity-30 animation-delay-75"></div>
          </div>
        </div>
...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  static getDerivedStateFromError(error: Error): ErrorBoundaryState {
    // Only catch database/timeout related errors
...
// Hook for components to throw errors to be caught by ErrorBoundary
export const useErrorHandler = () => {
  const [error, setError] = React.useState<Error | null>(null);

  const handleError = React.useCallback((error: Error) => {
    console.error('Throwing error to boundary:', error);
    setError(error);
  }, []);
