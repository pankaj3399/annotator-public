...
import { getAllAcceptedTasks } from "@/app/actions/task";
...
const TruncatedCell: React.FC<TruncatedCellProps> = ({ content, maxLength = 50 }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const shouldTruncate = content.length > maxLength;

  const displayContent = isExpanded ? content : shouldTruncate ? `${content.slice(0, maxLength)}...` : content;
...
export default function ProjectAnalytics({ projectId, projectName }: AnalyticsProps) {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        setLoading(true);
        const res = JSON.parse(await getAllAcceptedTasks(projectId)) as TaskData[];
        ...
        const processedData = res.map((task) => {
          try {
            ...
            return {
              ...
              submittedDate: format(new Date(task.created_at), 'MMM dd, yyyy HH:mm'),
  ...
  const avgTimePerTask = totalTasks > 0 
    ? (analyticsData.reduce((acc, curr) => acc + curr.timeTaken, 0) / totalTasks).toFixed(2)
    : 0;
        ...
        {analyticsData.length === 0 ? (
          ...
          <CardContent className="p-0"> {/* Remove padding from CardContent */}
            <div className="overflow-auto" style={{ maxHeight: '70vh' }}>
              <TableHeader>
                <TableRow>
                  <TableHead className="p-4 bg-white sticky top-0" style={{ width: '200px' }}>
                    Task Name
                  </TableHead>
                  <TableHead className="p-4 bg-white sticky top-0" style={{ width: '200px' }}>
                    Static Text
                  </TableHead>
                  <TableHead className="p-4 whitespace-nowrap bg-white sticky top-0">
                    Time Taken (mins)
                  </TableHead>
                  <TableHead className="p-4 whitespace-nowrap bg-white sticky top-0">
                    Submitted Date
                  ...
                  {analyticsData[0].dynamicFields.map((field, index) => (
                    <React.Fragment key={index}>
                      <TableHead 
                        className="p-4 bg-white sticky top-0"
                        style={{ width: '300px' }}
                      >
                        {field.name}
                      </TableHead>
              ...
              <TableBody>
                {analyticsData.map((row) => (
                  <TableRow key={row.taskId} className="align-top">
                    ...
                    <TableCell className="font-mono text-sm p-4 align-top whitespace-nowrap">
                      {row.annotator}
                    </TableCell>
                    <TableCell 
                      className="p-4 align-top"
