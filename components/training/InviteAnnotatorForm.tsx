...
export function InviteAnnotatorForm({
    ...
    getAllAnnotators()
      .then((jsonDataString) => {
        try {
          ...
          setAnnotators(
            parsedData.map((a: AnnotatorUser) => ({
          ...
          toast({
            variant: 'destructive',
            title: 'Error',
            description: 'Could not process annotator list.',
          });
      ...
      .catch((err) => {
        ...
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Could not load annotator list.',
        });
  ...
  const toggleAnnotator = (annotator: AnnotatorOption) => {
    ...
    const isSelected = selectedAnnotators.some(
      (a) => a.value === annotator.value
    );

    if (isSelected) {
      // Remove from selection
      setSelectedAnnotators((prev) =>
        prev.filter((a) => a.value !== annotator.value)
      );
    } else {
      // Add to selection
  ...
  const removeAnnotator = (annotatorId: string) => {
    setSelectedAnnotators((prev) =>
      prev.filter((a) => a.value !== annotatorId)
    );
  };
  ...
  async function handleInvite() {
    if (selectedAnnotators.length === 0) {
      toast({
        ...
        description: 'Please select at least one annotator.',
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: 'Success',
          description: `Invited ${selectedAnnotators.length} annotator${
            selectedAnnotators.length > 1 ? 's' : ''
          } successfully.`,
        });
        // Reset form
        ...
        toast({
          variant: 'destructive',
          title: 'Error',
          description: result.message || 'Failed to invite experts.',
        });
      }
    } catch (error: any) {
  ...
  return (
    <div className='space-y-4'>
      ...
      >
        ...
        Invite {selectedAnnotators.length > 0
          ? selectedAnnotators.length
          : ''}{' '}
        Annotator{selectedAnnotators.length !== 1 ? 's' : ''}
      </Button>
    </div>
  );
}
