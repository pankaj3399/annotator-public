...
export function TrainingWebinarList({ webinars: initialWebinars, trainingId }: TrainingWebinarListProps) {
    // State to manage webinars after status updates
    const [webinars, setWebinars] = useState<SerializedWebinarSession[]>(initialWebinars);
    const [isInviteModalOpen, setIsInviteModalOpen] = useState(false);
    const [selectedWebinar, setSelectedWebinar] = useState<SerializedWebinarSession | null>(null);
    
    const { data: session } = useSession();
    const userRole = (session?.user as { role?: string })?.role;
    const userName = (session?.user as { name?: string })?.name || 'Project Manager';
                        ...
                        <CardFooter className="flex flex-wrap items-center gap-2 pt-0">
                            ...
                            {isProjectManager && (
                                <StatusUpdateButton 
                                    ...
                                    onStatusUpdate={(newStatus) => handleStatusUpdate(webinar._id, newStatus)}
                                />
                            )}
                            ...
                            {!canJoinWebinar(webinar) && webinar.status !== 'completed' && webinar.status !== 'cancelled' && (
