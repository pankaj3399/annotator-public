...
const ProjectGuidelines = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isAiConfigured && aiConfig.apiKey) {
          ...
          toast.info(
            "Configure AI assistant to get automated responses",
            {
              action: {
                label: "Configure",
      ...
      if (response) {
        ...
        const aiResponseMessage = {
          ...
          sender: {
            ...
            image: "/ai-assistant-avatar.png", // Default AI avatar
          },
          content: response,
    ...
    try {
      ...
      try {
        ...
        if (!configResponse.ok) {
          console.log(`Warning: Could not save AI config. Status: ${configResponse.status}`);
  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    try {
      ...
      setUploading(false);
      // Clean up uploading files state
      setUploadingFiles([]);
    }
  };
    ...
    <Card className="w-full">
                  ...
                  <div className="text-sm">
                    {description ? (
                      ...
                      <p className="text-muted-foreground italic">
                        No description provided. Click edit to add project description and guidelines.
                      </p>
                    )}
                  </div>
                )}
              ...
              {!isAiConfigured && showAiSetupPrompt && messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-[400px] text-center px-4">
                  ...
                  <p className="text-muted-foreground max-w-md mb-6">
                    To get started with project guidelines, configure an AI assistant 
                    that will help manage conversations and answer questions about this project.
                  </p>
                    ...
                    <div className="space-y-4">
                      {messages.map((message) => (
                        ...
                        >
                          {message.sender._id !== session?.user.id && (
                            <div className="flex-shrink-0 mr-2">
                              <Avatar className="h-8 w-8">
                                ...
                                <AvatarFallback>
                                  {message.isAiMessage ? 'AI' : message.sender.name.charAt(0)}
                                </AvatarFallback>
                              </Avatar>
                            </div>
                          )}
                          ...
                          >
                            {message.sender._id !== session?.user.id && !message.isAiMessage && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFiles.map((file, index) => (
                      ...
                      >
                        ...
                        <span className="max-w-[150px] truncate mr-1">{file.fileName}</span>
                        <Button
                          type="button"
                          variant="ghost"
                          size="icon"
                  ...
                  <div className="space-y-2">
                    {uploadingFiles.map((uploadingFile, index) => (
                      <div key={index} className="flex items-center text-xs">
                        ...
                        <div className="flex-1 truncate">{uploadingFile.file.fileName}</div>
              ...
              {(isAiConfigured || messages.length > 0) && (
                <div className="p-3 border-t flex gap-2">
                  <Button
                    ...
                    title={!isAiConfigured ? "Configure AI assistant to send messages" : "Attach file"}
                  >
                    <PaperclipIcon className={`h-5 w-5 ${!isAiConfigured ? "opacity-50" : ""}`} />
                  ...
                  <Textarea
                    ...
                    placeholder={isAiConfigured ? "Type a message..." : "Configure AI assistant to send messages"}
                  ...
                  <Button
                    type="button"
                    size="icon"
                    onClick={!isAiConfigured ? () => setAiModalOpen(true) : handleSendMessage}
                    disabled={
                  ...
                  >
                    ...
                    <span className="sr-only">{!isAiConfigured ? "Configure AI" : "Send"}</span>
                  </Button>
                </div>
              )}
            </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground mt-1">
                            Uploaded {formatDistanceToNow(new Date(file.uploadedAt), {
                              addSuffix: true,
      ...
      <Dialog open={aiModalOpen} onOpenChange={setAiModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {isAiConfigured ? "AI Assistant Settings" : "Configure AI Assistant"}
            </DialogTitle>
            <DialogDescription>
              {isAiConfigured 
          ...
          <DialogFooter>
            ...
            >
              {isGeneratingAI ? (
                ...
                isAiConfigured ? "Update AI Assistant" : "Configure & Train AI"
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
