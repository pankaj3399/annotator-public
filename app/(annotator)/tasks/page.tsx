...
export default function ProjectDashboard() {
  ...
  const [benchmarkProposalProject, setBenchmarkProposalProject] = useState<Project | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const testProjectIds = new Set<string>(projectsWithTestsList.map((p: Project) => p._id))
    ...
    const fetchAssignedTests = async () => {
      try {
        if (session?.user?.id) {
          ...
          const assignedProjects = new Set<string>(testTasks.map((task) => task.project))
  ...
  const handleLabelClick = (label: LabelType) => {
    ...
    if (newSelectedLabels.length > 0) {
      filtered = filtered.filter((project) => {
        ...
        return newSelectedLabels.every((label) => projectLabels.includes(label) || project.labels.includes(label))
      })
    }
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let filtered = projects.filter((project) => project.name.toLowerCase().includes(query))
    if (selectedLabels.length > 0) {
      filtered = filtered.filter((project) => {
        ...
        return selectedLabels.every((label) => projectLabels.includes(label) || project.labels.includes(label))
      })
    }
    setFilteredProjects(filtered)
  }
