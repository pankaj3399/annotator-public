...
import { getTeamProjects, getRawAnnotationData } from '../../actions/dataScientist';
...
export default function DataPage(): React.ReactNode {
  ...
  const statuses = ['all', ...new Set(annotationData.map(item => item.status))];
            ...
            {projects.map((project) => (
              <button
                ...
                className={`p-4 border rounded-lg text-left hover:bg-gray-50 transition-colors ${
                  selectedProject === project._id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'
                }`}
      ...
      {selectedProject && (
        <>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {loading ? (
              <div className="p-6 text-center text-gray-500">Loading annotation data...</div>
              ...
              <div className="overflow-x-auto">
                    ...
                    {filteredData.slice(0, 50).map((item) => (
                      <tr key={item.taskId}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">
                          {item.content 
                            ? <pre className="text-xs overflow-hidden">{JSON.stringify(item.content).substring(0, 100)}...</pre>
                ...
                {filteredData.length > 50 && (
                  <div className="px-6 py-3 text-gray-500 text-sm bg-gray-50 border-t border-gray-200">
                    Showing 50 of {filteredData.length} results. Export to CSV to access all data.
                  </div>
                )}
              </div>
            )}
