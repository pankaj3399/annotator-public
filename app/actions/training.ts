...
export async function createWebinarAction(
    ...
    if (!MANAGEMENT_TOKEN) {
        ...
        return { success: false, message: "Server config error: Missing API token." };
    }
    if (!TEMPLATE_ID) {
        console.error("[Action Error] Missing TEMPLATE_ID");
        return { success: false, message: "Server config error: Missing template ID." };
    }

    // Validate IDs if provided
    if (args.trainingId && !mongoose.Types.ObjectId.isValid(args.trainingId)) {
        console.warn("[Action Warn] Invalid trainingId format provided:", args.trainingId);
    ...
    if (!args.instructorId || !mongoose.Types.ObjectId.isValid(args.instructorId)) {
        console.warn("[Action Warn] Invalid or missing instructorId:", args.instructorId);
    ...
    try {
        ...
        console.log("[Action Info] Attempting to create 100ms room with payload:", JSON.stringify(roomPayload));
        // *** END LOGGING ***
        ...
        // *** ADDED/ENHANCED LOGGING HERE ***
        console.log(`[Action Info] 100ms room creation response status: ${roomResponse.status}`);
        const responseBodyText = await roomResponse.text(); // Get text first
        console.log(`[Action Info] 100ms room creation RAW response body: ${responseBodyText}`);
        // *** END LOGGING ***

        if (!roomResponse.ok) {
            console.error(`[Action Error] Failed to create 100ms room (${roomResponse.status}). Body: ${responseBodyText}`);
            throw new Error(`Failed to create webinar room service (${roomResponse.status})`);
        }
        ...
        try {
            ...
            console.log("[Action Info] Parsed 100ms room creation success response body:", newRoomData);
        } catch (parseError) {
            console.error("[Action Error] Failed to parse 100ms success response as JSON.", parseError);
            throw new Error("Received non-JSON success response from webinar service API.");
        }
        // *** END LOGGING ***

        createdRoomId = newRoomData?.id; // Safely access 'id'
        if (!createdRoomId || typeof createdRoomId !== 'string') { // Check type too
            console.error("[Action Error] Valid 'id' (string) missing from 100ms API success response.", newRoomData);
            throw new Error("Failed to get valid room ID string from webinar service API.");
        }
        // *** ADDED/ENHANCED LOGGING HERE ***
        console.log(`[Action Info] Successfully extracted 100ms room ID: ${createdRoomId}`);
        // *** END LOGGING ***
        ...
        console.log("[Action Info] Prepared webinar subdocument data:", newWebinarSessionData);


        // --- Step 3: Database Operation (Conditional) ---
        if (args.trainingId) {
            // --- Path A: Update Existing Training ---
            console.log(`[Action Info] Attempting to add webinar ${newWebinarMongoId} to existing Training ${args.trainingId}`);
            const updateResult = await Training.updateOne(
                { _id: new Types.ObjectId(args.trainingId), project: new Types.ObjectId(args.projectId) },
                { $push: { webinars: newWebinarSessionData } }
            );

            if (updateResult.matchedCount === 0) {
                 console.warn(`[Action Warn] Training document not found for ID ${args.trainingId} and Project ${args.projectId}.`);
                 throw new Error('Training program not found for this project.');
            }
            if (updateResult.modifiedCount === 0) {
                 console.warn(`[Action Warn] Failed to modify Training document ${args.trainingId} (modifiedCount 0).`);
                 throw new Error('Failed to add webinar session to the training program.');
            }
            dbOperationSuccessful = true;
            console.log(`[Action Success] Successfully added webinar ${newWebinarMongoId} to Training ${args.trainingId}. DB Result:`, updateResult); // Log DB result

        } else {
            // --- Path B: Create New Training ---
            console.log(`[Action Info] Attempting to create new Training document for project ${args.projectId} with first webinar ${newWebinarMongoId}`);
            const newTraining = new Training({
                project: new Types.ObjectId(args.projectId),
                title: args.title || `Training for Project ${args.projectId}`,
                description: args.description || `Initial training setup with webinar '${args.title}'`,
            ...
            console.log(`[Action Success] Successfully created new Training document ${newTrainingId} for project ${args.projectId}.`);
        }
        ...
        return { success: true, message: "Webinar created successfully.", webinarId: newWebinarMongoId.toString() };
        ...
        if (createdRoomId && !dbOperationSuccessful) {
             console.warn(`[Action Cleanup] Attempting to disable created room ${createdRoomId} due to DB/processing error.`);
             try {
                 const disableResponse = await fetch(`${HMS_API_BASE_URL}/rooms/${createdRoomId}`, {
                     method: 'POST',
                     headers: { 'Authorization': `Bearer ${MANAGEMENT_TOKEN}`, 'Content-Type': 'application/json' },
                     body: JSON.stringify({ enabled: false }),
                 });
                 if(disableResponse.ok) {
                     console.log(`[Action Cleanup] Successfully disabled orphaned room ${createdRoomId}.`);
                 } else {
                      const disableErrorBody = await disableResponse.text();
                      console.error(`[Action Cleanup CRITICAL] Failed to disable orphaned room ${createdRoomId} (${disableResponse.status}). Body: ${disableErrorBody}`);
                 }
             } catch (disableError: any) {
                 console.error(`[Action Cleanup CRITICAL] Exception while trying to disable orphaned room ${createdRoomId}:`, disableError.message);
             }
        }
        return { success: false, message: error.message?.includes("webinar room service") ? "Failed to set up webinar service." : error.message || "Failed to create webinar." };
    }
}
...
export async function inviteAnnotatorAction(
    ...
    console.log(`[Action Start] inviteAnnotatorAction triggered with trainingId: ${trainingId}, annotatorId: ${annotatorId}`);
    const session = await getServerSession(authOptions);
    if (session?.user?.role !== 'project manager') {
         console.warn("[Action Denied] User is not a project manager.");
        return { success: false, message: "Permission denied." };
    }
    if (!mongoose.Types.ObjectId.isValid(trainingId) || !mongoose.Types.ObjectId.isValid(annotatorId)) {
        console.warn("[Action Warn] Invalid ID format provided.", { trainingId, annotatorId });
    ...
    try {
        ...
        if (updateResult.matchedCount === 0) {
             console.warn(`[Action Warn] Training not found for ID ${trainingId} during invite.`);
            return { success: false, message: "Training not found." };
        }
        if (updateResult.modifiedCount === 0 && updateResult.upsertedCount === 0) {
             console.log(`[Action Info] Annotator ${annotatorId} was already invited to training ${trainingId}.`);
             return { success: true, message: "Annotator already invited." };
        }
        console.log(`[Action Success] Invited annotator ${annotatorId} to training ${trainingId}`);
        interface TrainingWithProject { project: Types.ObjectId }
        const trainingDoc = await Training.findById(trainingId, { project: 1 }).lean<TrainingWithProject>();
        if (trainingDoc && trainingDoc.project) {
            const revalidationPath = `/projects/${trainingDoc.project.toString()}/training`;
             console.log(`[Action Info] Revalidating path: ${revalidationPath}`);
            revalidatePath(revalidationPath, 'page');
        } else {
            console.warn(`[Action Warn] Could not determine projectId for revalidation from trainingId ${trainingId}`);
        }
        return { success: true, message: "Annotator invited successfully." };
    } catch (error: any) {
